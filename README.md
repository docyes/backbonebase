...
All views have a DOM element `view.el` with additional meta-data set by the constructor; `data-cid` and `class`. 

```html
<div data-cid="view1" data-mid="views/myview"></div>
```

`data-cid` is derived from `view.cid` and `class` from `view.mid` if passed using the `view.toClassName` formatting function. The class attribute setting is additive; non-destructive.
